...
async function getCurrentWifiSSID() {
  try {
    // iwgetid 명령으로 현재 연결된 WiFi SSID 확인
    const { stdout } = await execAsync('iwgetid -r');
    const ssid = stdout.trim();
    return ssid;
  } catch (error) {
    console.error('Error getting WiFi SSID:', error.message);
    return null;
  }
}
...
async function isConnectedToRequiredWifi() {
  ...
  if (currentSSID === REQUIRED_WIFI_SSID) {
    return true;
  }
  if (currentSSID) {
    console.log(`⚠️ Connected to '${currentSSID}', but required WiFi is '${REQUIRED_WIFI_SSID}'`);
  } else {
    console.log(`⚠️ Not connected to WiFi or unable to detect SSID`);
  }
  return false;
}
...
async function processClips() {
  ...
  // WiFi 연결 확인
  const isConnected = await isConnectedToRequiredWifi();
  if (!isConnected) {
    console.log(`🚫 Skipping - not connected to required WiFi '${REQUIRED_WIFI_SSID}'`);
    return;
  }
  
  console.log(`✅ Connected to WiFi '${REQUIRED_WIFI_SSID}'`);
